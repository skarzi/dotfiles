...
ensure_dotfiles_venv_activated() {
  local dotfiles_venv="${1}"

  if [[ -z "${VIRTUAL_ENV+x}" ]] || [[ ! "${VIRTUAL_ENV}" =~ ^.*"${dotfiles_venv}"$ ]]; then
...
load_all_tmux_session_workspaces() {
  local workspaces_directory="${1:-${HOME}/.config/tmux/workspaces}"
...
ensure_dotfiles_venv_activated "${HOME}/dotfiles/.venv" \
  && load_all_tmux_session_workspaces "${HOME}/config/tmux/workspaces" \
  ;
deactivate
